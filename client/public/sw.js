...
const SW_VERSION = "1.0.0"; // Change this to trigger an update
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      ...
      return cache.addAll(urlsToCache).catch((err) => {
        console.error("[SW] Cache failed for some resources:", err);
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification("Vibes", options));
});
