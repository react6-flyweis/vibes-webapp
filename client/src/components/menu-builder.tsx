...
export default function MenuBuilder({ eventId, activeCategory, onCategoryChange, onAddItem }: MenuBuilderProps) {
  ...
  const claimMutation = useMutation({
    mutationFn: async (itemId: number) => {
      const response = await apiRequest("PATCH", `/api/menu-items/${itemId}/claim`, { userId: 2 }); // Using user 2 as current user for demo
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        {categories.map((category) => {
          ...
          return (
            <Button
              ...
              className={isActive 
                ? `${categoryColors[category.id as keyof typeof categoryColors]} text-white hover:opacity-90`
      ...
      >
        <div className="text-center">
          ...
          <p className="party-gray font-medium">Add new {activeCategory.slice(0, -1)} item</p>
          <p className="text-sm text-gray-500">Click to contribute to the menu</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8">
          <div className="text-party-gray">No {activeCategory} items yet. Be the first to add one!</div>
        </div>
      ) : (
            ...
            <div key={item.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                  ...
                  <div>
                    ...
                    {item.contributor && (
                      <p className="text-xs text-gray-500">
                        Added by <span className="font-medium">{item.contributor.name}</span>
                      </p>
                    )}
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  {item.status === "claimed" ? (
                    <>
                      <Badge className="bg-party-mint text-white">Claimed</Badge>
                      {item.claimedBy && (
                        <div className="flex -space-x-2">
                          <img 
                            className="w-6 h-6 rounded-full border-2 border-white" 
                            src={item.claimedBy.avatar || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-4.0.3&auto=format&fit=crop&w=24&h=24"} 
                            alt="Claimed by"
                          />
                        </div>
                      )}
                    ...
                    <Button 
                      ...
                      size="sm"
                    >
                      {claimMutation.isPending ? "Claiming..." : "I'll bring this"}
                    </Button>
                  )}
                </div>
              </div>
