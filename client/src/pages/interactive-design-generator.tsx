...
export default function InteractiveDesignGenerator() {
  ...
  // common, fixed brush size for the app
  const [brushSize] = useState(6);
  const lastPosRef = useRef<{ x: number; y: number } | null>(null);
  const ctxRef = useRef<CanvasRenderingContext2D | null>(null);
  // undo / redo stacks (store dataURL snapshots)
  ...
  const createSparkleEffect = (type: string) => {
    ...
    Array.from({ length: 8 }).forEach((_, index) => {
      setTimeout(() => {
        ...
        sparkle.className = `fixed pointer-events-none z-50 w-3 h-3 ${
  ...
  const generateMoodPalette = () => {
    const basePalette =
      moodPalettes[currentMood as keyof typeof moodPalettes] ||
      moodPalettes.energetic;
  ...
  const initializeSparkleEffect = () => {
    sparkleIntervalRef.current = setInterval(() => {
      ...
      const sparkle = document.createElement("div");
      sparkle.className =
        "fixed pointer-events-none z-50 w-2 h-2 bg-yellow-400 rounded-full animate-ping";
  ...
  // Canvas setup and drawing handlers
  useEffect(() => {
    // keep refs in sync
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (!ctx) return setupCanvas();
      const temp = document.createElement("canvas");
      const rect = canvas.getBoundingClientRect();
      temp.width = rect.width;
      temp.height = rect.height;
      const tctx = temp.getContext("2d");
      if (tctx) tctx.drawImage(canvas, 0, 0, temp.width, temp.height);
      setupCanvas();
      const newCtx = ctxRef.current;
      if (newCtx && tctx) newCtx.drawImage(temp, 0, 0, temp.width, temp.height);
    };
  ...
  const undo = () => {
    ...
    const tmp = document.createElement("canvas");
    tmp.width = currentRect.width;
    tmp.height = currentRect.height;
    const tctx = tmp.getContext("2d");
    if (tctx)
      tctx.drawImage(canvas, 0, 0, currentRect.width, currentRect.height);
    const currentData = tmp.toDataURL("image/png");
  ...
  const exportCanvas = () => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    // create a temporary canvas to export at displayed size
    const rect = canvas.getBoundingClientRect();
  ...
  const generateStory = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description:
          "Your personalized story is ready and shared with the group!",
      });
    },
    onError: () => {
      toast({
  ...
  const getMoodIcon = (mood: string) => {
    ...
    return (
      icons[mood as keyof typeof icons] || <Sparkles className="h-5 w-5" />
    );
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {colorPalette.map((color, index) => (
                        <div
                          ...
                          onClick={() => {
                            // Set brush color when a palette swatch is selected
                            setBrushColor(color);
                            // Add sparkle effect when color is selected
                            const sparkle = document.createElement("div");
                            sparkle.className =
                              "fixed pointer-events-none z-50 w-4 h-4 rounded-full animate-ping";
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {/* Using palette swatches to pick brush color; brush size is a fixed common size */}
                  <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Button className="w-full" onClick={generateMoodPalette}>
                ...
                >
                  ...
                  <CardContent>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      {mood === "energetic" &&
                        "Vibrant and dynamic colors for high-energy designs"}
                      {mood === "calm" &&
                        "Soothing blues and greens for peaceful compositions"}
                      {mood === "creative" &&
                        "Bold and expressive colors for artistic projects"}
                      {mood === "professional" &&
                        "Clean and sophisticated tones for business use"}
                      {mood === "romantic" &&
                        "Warm and passionate colors for intimate designs"}
                      {mood === "mysterious" &&
                        "Deep and enigmatic shades for intrigue"}
                      {mood === "playful" &&
                        "Fun and cheerful colors for joyful creations"}
                      {mood === "elegant" &&
                        "Refined and graceful tones for luxury appeal"}
                    </p>
                  </CardContent>
                </Card>
              ))}
                      ...
                      onClick={() => {
                        ...
                        Array.from({ length: 10 }).forEach((_, i) => {
                          setTimeout(() => {
                            const sparkle = document.createElement("div");
                            sparkle.className =
                              "fixed pointer-events-none z-50 w-3 h-3 bg-linear-to-r from-blue-400 to-purple-500 rounded-full animate-ping";
                            sparkle.style.left = `${Math.random() * 100}%`;
                            sparkle.style.top = `${Math.random() * 100}%`;
                            document.body.appendChild(sparkle);
                            setTimeout(() => {
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Event Context:{" "}
                    {eventContext?.title || "Sarah's Birthday Bash"}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="p-4 bg-linear-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 rounded-lg">
                  ...
                  <div className="space-y-3 max-h-80 overflow-y-auto">
                    {sharedContent.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-gray-400">
                          Create and share designs, stories, or palettes with
                          your group
                        </p>
                      </div>
                    ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                      Current palette automatically reflects group's collective
                      mood preferences
                    </p>
                    <Button
                      size="sm"
                  ...
                  <div className="p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                      Real-time design sharing with sparkle effects for each
                      group member action
                    </p>
                    <Button
                      size="sm"
                  ...
                  <div className="p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                      Stories incorporate event details and are automatically
                      shared with group
                    </p>
                    <Button
                      size="sm"
                      ...
                      onClick={() => {
                        ...
                        shareContentWithGroup({
                          type: "story",
                          content:
                            "AI-generated story about our amazing group celebration...",
                          mood: currentMood,
                          style: designStyle,
                        });
                      }}
                  ...
                  <div className="p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                      Unlock achievements together as group collaborates on
                      designs
                    </p>
                    <Button
                      size="sm"
