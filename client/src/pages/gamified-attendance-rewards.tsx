...
export default function GamifiedAttendanceRewards() {
  ...
  const claimRewardMutation = useMutation({
    mutationFn: async (rewardId: string) => {
      return await apiRequest("POST", `/api/gamification/rewards/${rewardId}/claim`, {});
    },
    onSuccess: () => {
      toast({
  ...
  const completechallengeMutation = useMutation({
    mutationFn: async (challengeId: string) => {
      return await apiRequest("POST", `/api/gamification/challenges/${challengeId}/complete`, {});
    },
    onSuccess: () => {
      toast({
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 max-w-4xl mx-auto leading-relaxed">
            Earn points, unlock achievements, and compete with friends through engaging event participation
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <Card className="bg-gradient-to-r from-purple-500 to-blue-500 text-white">
              <CardContent className="p-6 text-center">
                ...
                <div className="text-3xl font-bold">{(userStats as any).totalPoints || 2847}</div>
            ...
            <Card className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white">
              <CardContent className="p-6 text-center">
                ...
                <div className="text-3xl font-bold">{(userStats as any).rank || 12}</div>
            ...
            <Card className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white">
              <CardContent className="p-6 text-center">
                ...
                <div className="text-3xl font-bold">{(userStats as any).streak || 7}</div>
            ...
            <Card className="bg-gradient-to-r from-purple-500 to-pink-500 text-white">
              <CardContent className="p-6 text-center">
                ...
                <div className="text-3xl font-bold">{(userStats as any).badges || 23}</div>
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {[
                      { action: "Attended Sarah's Birthday", points: 50, time: "2 hours ago", icon: Calendar },
                      { action: "Shared event on social media", points: 25, time: "5 hours ago", icon: Share2 },
                      { action: "Early bird attendance bonus", points: 100, time: "1 day ago", icon: Timer },
                      { action: "Photo upload reward", points: 30, time: "2 days ago", icon: Camera }
                    ].map((activity, index) => {
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>You're close to leveling up!</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Earn points with these quick actions</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { action: "Check in to current event", points: 50, icon: MapPin, available: true },
                    { action: "Share event on social", points: 25, icon: Share2, available: true },
                    { action: "Upload event photo", points: 30, icon: Camera, available: true },
                    { action: "Invite a friend", points: 100, icon: Users, available: false }
                  ].map((action, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-xs text-center">{action.action}</span>
                        <Badge variant="outline" className="text-xs">+{action.points} pts</Badge>
                      </Button>
                    );
                  })}
                </div>
            ...
            {achievements && Array.isArray(achievements) && achievements.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {(achievements as any[]).map((achievement) => {
                  ...
                  return (
                    <Card key={achievement.id} className={`relative overflow-hidden ${achievement.unlocked ? 'bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20' : 'opacity-60'}`}>
                      <CardContent className="p-6">
                        <div className="flex items-start gap-4">
                          <div className={`p-3 rounded-full ${rarityColors[achievement.rarity]} flex items-center justify-center`}>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">Start attending events to unlock your first achievements!</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="challenges" className="space-y-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">Complete challenges to earn bonus rewards</p>
              </div>
            </div>

            {challenges && Array.isArray(challenges) && challenges.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {(challenges as any[]).filter(c => c.isActive).map((challenge) => (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-orange-600">{challenge.timeLeft}</span>
                          </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">New challenges will appear regularly. Check back soon!</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="leaderboard" className="space-y-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">See how you rank against other event enthusiasts</p>
              </div>
              <div className="flex gap-2">
            ...
            {leaderboard && Array.isArray(leaderboard) && leaderboard.length > 0 ? (
              <div className="space-y-4">
                {(leaderboard as any[]).map((entry, index) => (
                  <Card key={entry.userId} className={`${index < 3 ? 'border-2 border-yellow-400 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20' : ''}`}>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${
                            ...
                            index === 1 ? 'bg-gray-400' :
                            index === 2 ? 'bg-orange-600' : 'bg-gray-300 text-gray-700'
                          }`}>
                            {index < 3 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">The leaderboard will update as more users participate in events.</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="rewards" className="space-y-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">Manage and redeem your earned rewards</p>
              </div>
            </div>

            {userRewards && Array.isArray(userRewards) && userRewards.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {(userRewards as any[]).map((reward) => (
                  <Card key={reward.id} className={`relative ${reward.isRedeemed ? 'opacity-60' : ''}`}>
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">Attend events and complete challenges to earn rewards!</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
