...
export default function InteractiveLiveVibesInvite() {
  ...
  const generateVideo = async (options: any) => {
    try {
      const response = await apiRequest(
        `/api/invitations/${invitationId}/generate-video`,
        "POST",
        {
          guestId: "guest_001",
          options,
      ...
      toast({
        ...
        description:
          "Video generation request received. Processing in background.",
        variant: "default",
      });
    }
  };
    ...
    try {
      if (navigator.share) {
        ...
        toast({
          title: "Link Copied!",
          description:
            "Invitation link copied to clipboard. Share it with your friends!",
        });
      }
    } catch (error) {
      ...
      toast({
        ...
        description:
          "Use your browser's share options or copy the URL to share this invitation.",
        variant: "default",
      });
    }
  };
  ...
  const handleSaveToCalendar = () => {
    try {
      if (invitation) {
        ...
        if (
          window.confirm(
            "Add to Google Calendar? (Cancel to download .ics file instead)"
          )
        ) {
          ...
          link.href = url;
          link.download = `${invitation.eventTitle
            .replace(/[^a-z0-9]/gi, "_")
            .toLowerCase()}.ics`;
          link.click();
        ...
        toast({
          ...
          description:
            "Event has been added to your calendar with all invitation details.",
        });
      }
    } catch (error) {
      ...
      toast({
        ...
        description:
          "You can manually add this event to your calendar using the event details shown.",
        variant: "default",
      });
    }
  };
  ...
  const voteTrackMutation = useMutation({
    mutationFn: async (trackId: string) => {
      return await apiRequest(
        `/api/invitations/${invitationId}/vote-track`,
        "POST",
        { trackId }
      );
    },
  ...
  const mintNFTMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest(
        `/api/invitations/${invitationId}/mint-nft`,
        "POST",
        {}
      );
    },
  ...
  useEffect(() => {
    ...
    const drawVisualization = () => {
      ...
      for (let i = 0; i < bars; i++) {
        const barHeight =
          Math.random() * canvas.height * (isPlaying ? 0.8 : 0.2);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-4 mb-6">
            ...
            <div
              className="p-4 rounded-2xl shadow-2xl text-white"
              style={{
                background: `linear-gradient(135deg, ${invitationData.theme.secondaryColor}, ${invitationData.theme.primaryColor})`,
              }}
            >
          ...
          <TabsContent value="preview" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Explore the party space before you arrive!
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="relative bg-gray-900 rounded-lg overflow-hidden h-96 mb-6">
                  <div className="absolute inset-0 bg-linear-to-r from-purple-600/20 to-blue-600/20 flex items-center justify-center">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          toast({
                            title: "3D Tour",
                            description:
                              "Loading immersive venue experience...",
                          })
                        }
                      >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  {invitationData.virtualVenue.interactiveElements.map(
                      ...
                      <Button
                        ...
                        onClick={() =>
                          toast({
                            ...
                            description: `Focusing on ${element}`,
                          })
                        }
                      >
                ...
                <Card className="mb-6">
                  <CardHeader>
                    ...
                    <CardDescription>
                      AI-powered seat recommendation based on your preferences
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                      <Button
                        ...
                        onClick={() => {
                          const recommendedSeat =
                            Math.floor(Math.random() * 8) + 1; // VIP seats
                          setGuestProfile((prev) => ({
                            ...prev,
                            selectedSeat: recommendedSeat,
                          }));
                          toast({
                            title: "Perfect Match!",
                            description: `VIP seat ${recommendedSeat} recommended for party starters!`,
                          });
                        }}
                      >
                        🎉 Close to Action
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const recommendedSeat =
                            Math.floor(Math.random() * 8) + 25; // Back seats
                          setGuestProfile((prev) => ({
                            ...prev,
                            selectedSeat: recommendedSeat,
                          }));
                          toast({
                            title: "Perfect Match!",
                            description: `Quiet seat ${recommendedSeat} perfect for observers!`,
                          });
                        }}
                      >
                        🧘 Chill Vibes
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const recommendedSeat =
                            Math.floor(Math.random() * 12) + 13; // Middle area
                          setGuestProfile((prev) => ({
                            ...prev,
                            selectedSeat: recommendedSeat,
                          }));
                          toast({
                            title: "Perfect Match!",
                            description: `Social seat ${recommendedSeat} great for mingling!`,
                          });
                        }}
                      >
                        💬 Social Zone
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const availableSeats = [...Array(32)]
                            .map((_, i) => i + 1)
                            .filter(
                              (seat) => ![3, 7, 12, 18, 23, 27].includes(seat)
                            );
                          const randomSeat =
                            availableSeats[
                          ...
                          setGuestProfile((prev) => ({
                            ...prev,
                            selectedSeat: randomSeat,
                          }));
                          toast({
                            title: "Surprise Me!",
                            description: `Random seat ${randomSeat} selected!`,
                          });
                        }}
                      >
                        🎲 Surprise Me
                    ...
                    <div className="relative bg-gray-100 dark:bg-gray-800 rounded-lg p-6 mb-4">
                      ...
                      <div className="grid grid-cols-8 gap-2 mt-28">
                        {[...Array(32)].map((_, index) => {
                          ...
                          const isVIP = index < 8;
                          const isSelected =
                            guestProfile.selectedSeat === seatNumber;
                          const isOccupied = [3, 7, 12, 18, 23, 27].includes(
                            seatNumber
                          );
                          const isRecommended =
                            guestProfile.vibeType === "party-starter" && isVIP;
                    ...
                    {guestProfile.selectedSeat && (
                      <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                          {guestProfile.selectedSeat <= 8
                            ? "Front row energy! Perfect for dancing."
                            : guestProfile.selectedSeat <= 24
                            ? "Great social spot with good views."
                ...
                <div className="relative h-96 bg-linear-to-br from-purple-900 via-blue-900 to-indigo-900 rounded-lg overflow-hidden">
                  <div
                  ...
                  >
                    ...
                    >
                      {/* Main Hall */}
                      <div
                        className="absolute inset-4 bg-linear-to-b from-purple-800/80 to-purple-900/80 rounded-lg backdrop-blur-xs border border-purple-400/30 cursor-pointer"
                        style={{ transform: "rotateY(0deg) translateZ(20px)" }}
                        onClick={() =>
                          toast({
                            title: "Main Hall",
                            description:
                              "DJ Stage & Dance Floor - Capacity: 150 guests",
                          })
                        }
                      >
                        <div className="p-4 h-full flex flex-col justify-between relative">
                          ...
                          <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 w-20 h-12 border-2 border-yellow-400/50 rounded bg-yellow-400/20">
                            <div className="text-xs text-yellow-200 text-center mt-3">
                              Dance Floor
                            </div>
                          </div>
                              ...
                              <div
                                ...
                                onClick={(e) => {
                                  ...
                                  toast({
                                    ...
                                    description: `Round table for 12 guests - Premium location`,
                                  });
                                }}
                              >
                                {/* Table */}
                                <div className="w-8 h-8 bg-amber-600 rounded-full border-2 border-amber-400 shadow-lg">
                                  <div className="text-xs text-amber-100 text-center mt-1">
                                    {tableNum}
                                  </div>
                                </div>
                                {/* Chairs around table */}
                            ...
                            {[9, 10, 11, 12].map((tableNum) => (
                              <div
                                ...
                                onClick={(e) => {
                                  ...
                                  toast({
                                    title: `Table ${tableNum}`,
                                    description: `Round table for 10 guests - DJ view`,
                                  });
                                }}
                              >
                                {/* Table */}
                                <div className="w-6 h-6 bg-amber-600 rounded-full border-2 border-amber-400 shadow-lg">
                                  <div className="text-xs text-amber-100 text-center">
                                    {tableNum}
                                  </div>
                                </div>
                                {/* Chairs around table */}
                          ...
                          <div
                            ...
                            onClick={(e) => {
                              ...
                              toast({
                                title: "VIP Section",
                                description:
                                  "Exclusive seating for premium guests",
                              });
                            }}
                          >
                      ...
                      {/* VIP Lounge */}
                      <div
                        className="absolute inset-4 bg-linear-to-b from-yellow-800/80 to-yellow-900/80 rounded-lg backdrop-blur-xs border border-yellow-400/30 cursor-pointer"
                        style={{ transform: "rotateY(90deg) translateZ(20px)" }}
                        onClick={() =>
                          toast({
                            title: "VIP Lounge",
                            description:
                              "Exclusive area with premium amenities",
                          })
                        }
                      >
                        <div className="p-4 h-full flex flex-col justify-between relative">
                            ...
                            {[13, 14].map((tableNum) => (
                              <div
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toast({
                                    title: `VIP Table ${tableNum}`,
                                    description: `Premium table for 10 guests - Bottle service included`,
                                  });
                                }}
                              >
                                {/* VIP Table */}
                                <div className="w-10 h-10 bg-yellow-600 rounded-full border-2 border-yellow-400 shadow-lg">
                                  <div className="text-xs text-yellow-100 text-center mt-2">
                                    {tableNum}
                                  </div>
                                </div>
                          ...
                          <div className="absolute top-12 right-4 space-y-2">
                            <div
                              className="w-8 h-4 bg-yellow-700 rounded border border-yellow-500 cursor-pointer"
                              onClick={(e) => {
                                e.stopPropagation();
                                toast({
                                  title: "Lounge Seating",
                                  description: "Luxury sofas for 6 guests",
                                });
                              }}
                            >
                          ...
                          {/* VIP Bar */}
                          <div
                            className="absolute bottom-8 left-1/2 transform -translate-x-1/2 w-12 h-6 bg-yellow-600 rounded border border-yellow-400 cursor-pointer"
                            onClick={(e) => {
                              e.stopPropagation();
                              toast({
                                title: "VIP Bar",
                                description:
                                  "Premium bottle service and exclusive cocktails",
                              });
                            }}
                          >
                            <div className="text-xs text-yellow-100 text-center mt-1">
                              VIP BAR
                            </div>
                          </div>
                      ...
                      {/* Outdoor Patio */}
                      <div
                        className="absolute inset-4 bg-linear-to-b from-green-800/80 to-green-900/80 rounded-lg backdrop-blur-xs border border-green-400/30 cursor-pointer"
                        style={{
                        ...
                        onClick={() =>
                          toast({
                            title: "Outdoor Patio",
                            description:
                              "Garden setting with fresh air and city views",
                          })
                        }
                      >
                        <div className="p-4 h-full flex flex-col justify-between relative">
                            ...
                            {[15, 16, 17, 18].map((tableNum) => (
                              <div
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toast({
                                    title: `Patio Table ${tableNum}`,
                                    description: `High-top table for 6 guests - Fresh air dining`,
                                  });
                                }}
                              >
                                {/* High Table */}
                          ...
                          <div className="absolute top-12 right-4 space-y-3">
                            <div
                              className="w-10 h-6 bg-green-700 rounded border border-green-500 cursor-pointer"
                              onClick={(e) => {
                                e.stopPropagation();
                                toast({
                                  title: "Garden Lounge",
                                  description:
                                    "Outdoor sectional seating for 8 guests",
                                });
                              }}
                            >
                              <div className="text-xs text-green-100 text-center mt-1">
                                LOUNGE
                              </div>
                            </div>
                            <div
                          ...
                          <div
                            ...
                            onClick={(e) => {
                              ...
                              toast({
                                title: "Garden Bar",
                                description:
                                  "Outdoor cocktail service with fresh air",
                              });
                            }}
                          >
                            <div className="text-xs text-green-100 text-center mt-1">
                      ...
                      {/* Photo Booth Area */}
                      <div
                        className="absolute inset-4 bg-linear-to-b from-pink-800/80 to-pink-900/80 rounded-lg backdrop-blur-xs border border-pink-400/30 cursor-pointer"
                        style={{
                        ...
                        onClick={() =>
                          toast({
                            title: "Photo Booth",
                            description:
                              "Professional photo booth with props and instant prints",
                          })
                        }
                      >
                  ...
                  <div className="absolute bottom-4 right-4 flex gap-2">
                    <Button
                      ...
                      onClick={() =>
                        toast({
                          title: "360° View",
                          description: "Exploring venue from all angles",
                        })
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        toast({
                          title: "Focus Mode",
                          description: "Zooming into venue details",
                        })
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        toast({
                          title: "Virtual Tour",
                          description: "Starting guided venue walkthrough",
                        })
                      }
                    >
                  ...
                  <div className="absolute top-4 left-4 bg-black/50 backdrop-blur-xs rounded-lg p-3 text-white">
                    <h3 className="font-bold text-sm">
                      Jordan's Birthday Venue
                    </h3>
                    <p className="text-xs text-gray-300">
                      4 Interactive Areas • 360° Exploration
                    </p>
                    <div className="flex gap-1 mt-2 flex-wrap">
                      ...
                      <span className="text-xs bg-pink-500/30 px-2 py-1 rounded">
                        Photos
                      </span>
                    </div>
                  </div>
          ...
          <TabsContent value="explore" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Get a taste of the night's vibe and vote for your favorites!
                </CardDescription>
              </CardHeader>
              <CardContent>
                {/* Audio Visualizer */}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <Button
                              ...
                              onClick={() =>
                                handlePlayTrack(track.id, track.preview)
                              }
                            >
                              {selectedTrack === track.id && isPlaying ? (
          ...
          <TabsContent value="interact" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Let everyone know what energy you're bringing!
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                  {vibeTypes.map((vibe) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`p-2 rounded-full text-white ${vibe.color}`}
                        >
                      ...
                      style={{
                        backgroundColor:
                          guestProfile.rsvpStatus === "confirmed"
                            ? `${invitationData.theme.primaryColor}20`
                            : "#ef444420",
                        color:
                          guestProfile.rsvpStatus === "confirmed"
            ...
            {/* Smart Contract RSVP */}
            {invitationData.smartContract.enabled &&
              guestProfile.rsvpStatus === "confirmed" && (
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                    ...
                    <CardDescription>
                      Secure your spot with NFT verification
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {!guestProfile.nftVerified ? (
                      <div className="text-center">
                        <p className="text-gray-600 dark:text-gray-400 mb-4">
                          Mint your NFT RSVP token for ultimate authenticity and
                          exclusive perks!
                        </p>
                        <Button
                          size="lg"
            ...
            {guestProfile.rsvpStatus === "confirmed" && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Special rewards for confirming your attendance!
                  </CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {invitationData.rsvpDetails.confirmed} confirmed •{" "}
                  {invitationData.rsvpDetails.pending} pending
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card className="mb-6">
              <CardHeader>
                ...
                <CardDescription>
                  Real-time guest emotion tracking and venue energy
                </CardDescription>
              </CardHeader>
              <CardContent>
                {/* Mood Selection */}
        ...
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>
              Custom AI-generated video message just for you
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              {/* Video Preview */}
              <div className="relative aspect-video bg-linear-to-br from-purple-900 to-blue-900 rounded-lg overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <Button
                    size="lg"
                    className="bg-white/20 backdrop-blur-xs text-white border-white/30"
                    onClick={() =>
                      generateVideo({
                        includeHostMessage: true,
                        includeVenueTour: true,
                        includeMusicPreview: true,
                      })
                    }
                  >
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              Most active and engaged party guests
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {[
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full ${guest.color} flex items-center justify-center text-white font-bold text-sm`}
                  >
                    {index + 1}
                  </div>
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              Experience the party before you arrive
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              {/* AR Preview Window */}
              <div className="relative aspect-square bg-linear-to-br from-cyan-900 to-purple-900 rounded-lg overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center text-white">
                    ...
                    <p className="text-sm opacity-80">
                      Scan to preview the venue in AR
                    </p>
                  </div>
                </div>
        ...
        <Card className="mt-8">
          <CardContent className="p-6 text-center">
            ...
            <div className="flex gap-4 justify-center">
              <Button variant="outline" onClick={handleShareInvitation}>
                <Share2 className="w-4 h-4 mr-2" />
                Share with Friends
              </Button>
              <Button variant="outline" onClick={handleSaveToCalendar}>
                <Download className="w-4 h-4 mr-2" />
                Save to Calendar
              </Button>
            </div>
