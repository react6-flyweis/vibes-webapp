...
export default function LoyaltyRewards() {
  ...
  const loyaltyTiers: LoyaltyTier[] = [
    ...
    },
    {
      ...
      benefits: ["15% event discounts", "VIP customer support", "Exclusive events", "Free cancellation"],
      icon: Crown
    },
    {
      name: "Platinum",
      ...
      benefits: ["20% event discounts", "Personal concierge", "Early access", "Complimentary upgrades"],
      icon: Award
    }
  ];
  ...
  const getCurrentTier = () => {
    ...
    return loyaltyTiers.reverse().find(tier => userStats.totalPoints >= tier.minPoints) || loyaltyTiers[0];
  };

  const getNextTier = () => {
    if (!userStats) return loyaltyTiers[1];
    return loyaltyTiers.find(tier => userStats.totalPoints < tier.minPoints) || loyaltyTiers[loyaltyTiers.length - 1];
  };
  ...
  const getProgressToNextTier = () => {
    ...
    const progress = ((userStats.totalPoints - currentTier.minPoints) / (nextTier.minPoints - currentTier.minPoints)) * 100;
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <Card className="bg-white/10 backdrop-blur border-white/20">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white">{userStats?.availablePoints?.toLocaleString()}</div>
          ...
          <Card className="bg-white/10 backdrop-blur border-white/20">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white">{userStats?.lifetimeEarned?.toLocaleString()}</div>
          ...
          <Card className="bg-white/10 backdrop-blur border-white/20">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white">{userStats?.referralsCount}</div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <Card className="mb-8 bg-gradient-to-r from-purple-800 to-blue-800 border-white/20">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-white">
                  {React.createElement(currentTier.icon, { className: "h-6 w-6 text-yellow-400" })}
                ...
                {currentTier !== nextTier && (
                  <div className="mb-4">
                    <div className="flex justify-between text-sm mb-2">
                      <span className="text-purple-100">{currentTier.name}</span>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {rewards.map((reward: Reward) => (
                <Card key={reward.id} className="bg-white/10 backdrop-blur border-white/20">
                  <div className="relative">
                    ...
                    <Badge className={`absolute top-2 right-2 ${
                      reward.category === 'exclusive' ? 'bg-purple-600' :
                      reward.category === 'discount' ? 'bg-green-600' :
                      reward.category === 'experience' ? 'bg-blue-600' :
                      'bg-orange-600'
                    }`}>
                      {reward.category}
                    </Badge>
                  ...
                  <CardContent className="p-4">
                    ...
                    <p className="text-purple-100 text-sm mb-3">{reward.description}</p>
                    
                    <div className="flex justify-between items-center mb-3">
                      <div className="flex items-center gap-1">
                        <Coins className="h-4 w-4 text-yellow-400" />
                        <span className="text-white font-semibold">{reward.pointsCost.toLocaleString()}</span>
                      </div>
                      {reward.expiresAt && (
                    ...
                    >
                      {reward.claimed ? (
                        ...
                        <>
                          ...
                          Need {(reward.pointsCost - (userStats?.availablePoints || 0)).toLocaleString()} more
                        </>
                      ) : (
                        <>
              ...
              <CardContent>
                <div className="space-y-4">
                  {loyaltyTiers.map((tier, index) => {
                    ...
                    const isUnlocked = (userStats?.totalPoints || 0) >= tier.minPoints;
                    
                    return (
                      <div key={tier.name} className={`flex items-center gap-3 p-3 rounded-lg ${
                        isCurrentTier ? 'bg-purple-600/30 border border-purple-400' : 
                        isUnlocked ? 'bg-green-600/20' : 'bg-gray-600/20'
                      }`}>
                        <div className={`p-2 rounded-full ${
                          isUnlocked ? tier.color : 'bg-gray-400 text-gray-600'
                        }`}>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {isCurrentTier && <Badge className="bg-purple-600">Current</Badge>}
                            {isUnlocked && !isCurrentTier && <Unlock className="h-4 w-4 text-green-400" />}
                          </div>
                  ...
                  {recentActivity.slice(0, 5).map((activity: any, index: number) => (
                    <div key={index} className="flex justify-between items-center py-2">
                      <div>
                        ...
                        <div className="text-purple-200 text-xs">{new Date(activity.date).toLocaleDateString()}</div>
                      </div>
                  ...
                  {leaderboard.slice(0, 5).map((user: any, index: number) => (
                    <div key={index} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-purple-200 text-xs">{user.points.toLocaleString()} points</div>
                      </div>
                    </div>
                  ))}
                </div>
