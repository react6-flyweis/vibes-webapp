...
export default function InteractiveSeatTracker() {
  ...
  const [viewMode, setViewMode] = useState<
    "floor-plan" | "table-list" | "guest-list" | "3d-view"
  >("floor-plan");
    ...
    const sampleTables: Table[] = [
      {
        ...
        seats: Array.from({ length: 8 }, (_, i) => ({
          ...
          guestName:
            i < 3 ? `Guest ${i + 1}` : i < 6 ? `Reserved ${i + 1}` : undefined,
      ...
      },
      {
        ...
        reservationNotes:
          "Birthday celebration for grandmother, wheelchair accessible needed",
  ...
  const handleSeatClick = (seat: Seat) => {
    ...
    setInteractionHeatmap(
      (prev) => new Map(prev.set(seat.id, (prev.get(seat.id) || 0) + 1))
    );
  };
  ...
  const handleTableReservation = (
    ...
    setTables((prev) =>
      prev.map((table) =>
        table.id === tableId ? { ...table, ...reservationData } : table
      )
    );
  ...
  const addNewTable = (tableData: {
    ...
    const newTable: Table = {
      ...
      width:
        tableData.type === "round" ? 100 : tableData.type === "bar" ? 150 : 120,
      height:
        tableData.type === "round" ? 100 : tableData.type === "bar" ? 80 : 80,
    ...
    const suggestions = [
      {
        ...
        description:
          "Place guests with similar interests at adjacent tables for better networking",
        impact: "High",
        tables: [1, 2, 3],
        efficiency: 92,
      },
  ...
  const getHeatmapIntensity = (seatId: string) => {
    ...
    const maxInteractions = Math.max(
      ...Array.from(interactionHeatmap.values()),
      1
    );
  ...
  useEffect(() => {
    if (animationMode === "allocation") {
      const interval = setInterval(() => {
        const availableSeats = tables.flatMap((t) =>
          t.seats.filter((s) => s.status === "available")
        );
  ...
  const filteredSeats = tables
    .flatMap((table) => table.seats)
    .filter((seat) => filterStatus === "all" || seat.status === filterStatus);
                ...
                <CardTitle className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setIsTableManagementMode(!isTableManagementMode)
                      }
                      className={
                  ...
                  <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    <div className="flex items-center gap-2 text-blue-700 dark:text-blue-300">
                      ...
                      <span className="text-sm font-medium">
                        Table Edit Mode Active
                      </span>
                    </div>
                    <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                      Drag tables to move them around. Click tables to reserve
                      them or delete unwanted tables.
                    </p>
                  </div>
                )}
              </CardHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <Button
                          variant={
                            animationMode === "heatmap" ? "default" : "outline"
                          }
                          size="sm"
                          onClick={() =>
                            setAnimationMode(
                              animationMode === "heatmap" ? "none" : "heatmap"
                            )
                          }
                          className="text-xs"
                        >
                        ...
                        <Button
                          variant={
                            animationMode === "mood" ? "default" : "outline"
                          }
                          size="sm"
                          onClick={() =>
                            setAnimationMode(
                              animationMode === "mood" ? "none" : "mood"
                            )
                          }
                          className="text-xs"
                        >
                        ...
                        <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex gap-1">
                              {["warm", "cool", "vibrant", "calm"].map(
                                (color) => (
                                  <Button
                                    key={color}
                                    variant={
                                      moodLighting.color === color
                                        ? "default"
                                        : "outline"
                                    }
                                    size="sm"
                                    onClick={() =>
                                      setMoodLighting((prev) => ({
                                        ...prev,
                                        color,
                                      }))
                                    }
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs">
                              {Math.round(moodLighting.intensity * 100)}%
                            </span>
                          </div>
                        </div>
                      )}
                    ...
                    {/* 3D Scene */}
                    <div
                      className="relative bg-linear-to-b from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800 rounded-lg overflow-hidden h-96"
                      style={{
                        perspective: "1000px",
                      ...
                      onMouseMove={(e) => {
                        if (e.buttons === 1) {
                          ...
                          setCameraAngle({
                            x: Math.max(
                              -90,
                              Math.min(90, cameraAngle.x + y * 10)
                            ),
                    ...
                    >
                      {/* Floor */}
                      <div
                        className="absolute inset-0 bg-linear-to-br from-amber-100 via-amber-50 to-yellow-100 dark:from-amber-900 dark:via-amber-800 dark:to-yellow-800"
                        style={{
                          transform: `rotateX(${
                            90 + cameraAngle.x
                          }deg) rotateY(${cameraAngle.y}deg) translateZ(-50px)`,
                          transformOrigin: "center center",
                          backgroundImage:
                            "radial-gradient(circle at 25% 25%, rgba(139, 69, 19, 0.1) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(160, 82, 45, 0.1) 0%, transparent 50%)",
                        }}
                      />
                          ...
                          <div key={table.id} className="absolute">
                            ...
                            {Array.from({
                              ...
                              return (
                                <div
                                  key={legIndex}
                                  className="absolute bg-linear-to-b from-amber-700 to-amber-900"
                                  style={{
                            ...
                            {table.seats.map((seat, seatIndex) => {
                              const angle =
                                (seatIndex * 2 * Math.PI) / table.seats.length;
                              const chairDistance =
                                table.type === "round"
                                  ? table.width / 2 + 30
                              ...
                              const chairX =
                                tableX +
                                ((Math.cos(angle) * chairDistance) / 800) * 100;
                              const chairY =
                                tableY +
                                ((Math.sin(angle) * chairDistance) / 500) * 100;

                              const heatmapIntensity =
                                animationMode === "heatmap"
                                  ? getHeatmapIntensity(seat.id)
                                  : 0;
                              const moodOverlay =
                                animationMode === "mood"
                                  ? getMoodLightingStyle()
                                  : "rgba(0,0,0,0)";
                              ...
                              const chairColor =
                                ...
                                seat.status === "available"
                                  ...
                                  ? `from-orange-${Math.round(
                                      300 + heatmapIntensity * 200
                                    )} to-red-${Math.round(
                                      400 + heatmapIntensity * 300
                                    )}`
                                  : seat.status === "available"
                              ...
                              return (
                                <div key={seat.id} className="absolute">
                                  {/* Chair Seat */}
                                  <div
                                    className={`absolute bg-linear-to-br ${chairColor} shadow-md cursor-pointer transition-all duration-200 hover:scale-110`}
                                    style={{
                                      left: `${chairX}%`,
                                      top: `${chairY}%`,
                                      width: "3%",
                                      ...
                                      <div
                                        ...
                                        style={{
                                          ...
                                          background: `radial-gradient(circle, rgba(255, ${
                                            255 - heatmapIntensity * 255
                                          }, 0, ${heatmapIntensity}) 0%, transparent 80%)`,
                                  ...
                                  {/* Chair Back */}
                                  <div
                                    className={`absolute bg-linear-to-t ${chairColor} shadow-md`}
                                    style={{
                                      left: `${chairX}%`,
                                      top: `${chairY - 1}%`,
                                      width: "3%",
                                  ...
                                  {Array.from({ length: 4 }).map(
                                    (_, legIndex) => {
                                      ...
                                      return (
                                        <div
                                          key={legIndex}
                                          className="absolute bg-linear-to-b from-gray-600 to-gray-800"
                                          style={{
                    ...
                    <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                      <p>
                        Click and drag to rotate • Use zoom controls to get
                        closer • Click tables and chairs to interact
                      </p>
                    </div>
                  </div>
                )}
                          ...
                          {table.type === "round" ? (
                            <circle
                              ...
                              className={`${
                                ...
                                draggedTable === table.id ? "opacity-70" : ""
                              }`}
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              onMouseDown={(e) =>
                                handleTableDragStart(table.id, e)
                              }
                              onTouchStart={(e) =>
                                handleTableDragStart(table.id, e)
                              }
                            ...
                            <h3 className="font-semibold flex items-center gap-2">
                              ...
                              {table.reservedFor && (
                                <Badge className="bg-purple-600">
                                  {table.reservationType === "vip" && (
                                    <Crown className="w-3 h-3 mr-1" />
                                  )}
                                  {table.reservationType === "family" && (
                                    <Heart className="w-3 h-3 mr-1" />
                                  )}
                                  {table.reservationType === "business" && (
                    ...
                    <div className="text-2xl font-bold text-yellow-600">
                      {
                        tables
                          .flatMap((t) => t.seats)
                          .filter((s) => s.status === "reserved").length
                      }
                    </div>
                    ...
                    <div className="text-2xl font-bold text-red-600">
                      {
                        tables
                          .flatMap((t) => t.seats)
                          .filter((s) => s.status === "occupied").length
                      }
                    </div>
                    ...
                    <div>
                      ...
                      <Select
                        onValueChange={(guestId) =>
                          handleSeatAssignment(guestId, selectedSeat.id)
                        }
                      >
                        <SelectTrigger>
            ...
            {selectedTable && (
              <TableReservationForm
                ...
                onSave={(reservationData) =>
                  handleTableReservation(selectedTable.id, reservationData)
                }
...
function TableReservationForm({
  ...
  const [reservationType, setReservationType] = useState<
    "vip" | "family" | "business" | "special"
  >(table.reservationType || "special");
  const [reservationNotes, setReservationNotes] = useState(
    table.reservationNotes || ""
  );
  const [minimumSpend, setMinimumSpend] = useState(
    table.minimumSpend?.toString() || ""
  );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value: any) => setReservationType(value)}
          >
      ...
      <Card className="p-4 border-purple-200 bg-purple-50 dark:bg-purple-900/20">
        <div className="flex items-start gap-3">
          <div className="shrink-0 p-2 bg-purple-600 text-white rounded-lg">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-purple-700 dark:text-purple-300 mt-1">
              Table {table.number} reserved for{" "}
              {reservedFor || "special guests"}
            </p>
            {minimumSpend && (
