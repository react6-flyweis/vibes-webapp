...
export default function LiveMusicVoting() {
  ...
  const [selectedSource, setSelectedSource] = useState<'spotify' | 'apple' | 'youtube'>('spotify');
  ...
  useEffect(() => {
    if (isLiveMode) {
      const interval = setInterval(() => {
        // Simulate new votes
        setSongSuggestions(prev => prev.map(song => ({
          ...song,
        ...
        setMusicStats(prev => ({
          ...
          activeGuests: Math.max(15, prev.activeGuests + Math.floor(Math.random() * 3 - 1))
        }));
      }, 3000);
  ...
  const voteSong = (songId: string) => {
    ...
    toast({
      ...
      description: `${targetSong.hasVoted ? "Removed vote from" : "Voted for"} "${targetSong.title}"`,
    });
  };
  ...
  const suggestSong = (searchResult: any) => {
    if (suggestionNote.length < 5) {
      toast({
        title: "Add a note",
        description: "Please add a short note about why you want this song played.",
        variant: "destructive"
      });
      return;
    }
  ...
  const getSourceIcon = (source: string) => {
    switch (source) {
      case 'spotify': return <div className="w-4 h-4 bg-green-500 rounded-full" />;
      case 'apple': return <div className="w-4 h-4 bg-gray-900 rounded-full" />;
      case 'youtube': return <div className="w-4 h-4 bg-red-500 rounded-full" />;
      default: return <Music className="w-4 h-4" />;
    }
  };

  const sortedSuggestions = [...songSuggestions].sort((a, b) => b.votes - a.votes);
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="text-center mb-8">
        ...
        <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
          Let your guests shape the music! Real-time song suggestions, voting, and DJ integration 
          create the perfect soundtrack for your event.
        </p>
      </div>
      ...
      <Card className="mb-8 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
        <CardContent className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-purple-600">{musicStats.totalSuggestions}</div>
              <div className="text-sm text-gray-600 dark:text-gray-300">Song Suggestions</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-pink-600">{musicStats.totalVotes}</div>
              <div className="text-sm text-gray-600 dark:text-gray-300">Total Votes</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-blue-600">{musicStats.activeGuests}</div>
              <div className="text-sm text-gray-600 dark:text-gray-300">Active Guests</div>
            </div>
            <div>
              <div className="text-lg font-bold text-green-600">{musicStats.topGenre}</div>
              <div className="text-sm text-gray-600 dark:text-gray-300">Top Genre</div>
            </div>
            <div>
              <div className="flex items-center justify-center gap-2">
                {isLiveMode && <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse" />}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {sortedSuggestions.map((song, index) => (
                <div key={song.id} className={`p-4 border rounded-lg transition-all hover:shadow-md ${
                  song.isPlaying ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''
                }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <Heart className={`h-4 w-4 mr-1 ${song.hasVoted ? 'fill-current' : ''}`} />
                            Vote
                          </Button>
              ...
              {searchResults.length > 0 && searchQuery && (
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {searchResults.map((result) => (
                    <div key={result.id} className="flex items-center gap-3 p-2 border rounded hover:bg-gray-50 dark:hover:bg-gray-800">
                      <img src={result.albumArt} alt="" className="w-10 h-10 rounded" />
                      <div className="flex-1 min-w-0">
                        <div className="font-medium truncate">{result.title}</div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">{result.artist}</div>
                      </div>
                      <Button 
                        size="sm" 
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={djSettings.maxSuggestionsPerGuest.toString()} onValueChange={(value) => 
                    setDjSettings(prev => ({ ...prev, maxSuggestionsPerGuest: parseInt(value) }))
                  }>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{musicStats.averageVotesPerSong}</span>
              </div>
              <div className="flex justify-between">
