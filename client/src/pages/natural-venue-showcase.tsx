...
interface DecorElement {
  id: string;
  type: 'centerpiece' | 'lighting' | 'backdrop' | 'florals' | 'arch' | 'draping';
  name: string;
  price: number;
  description: string;
...
export default function NaturalVenueShowcase() {
  ...
  const [selectedShowcase, setSelectedShowcase] = useState<VenueShowcase | null>(null);
  const [viewMode, setViewMode] = useState<'gallery' | 'virtual' | 'ar'>('gallery');
  ...
  const filteredShowcases = showcases.filter(showcase => {
    const matchesTheme = filterTheme === 'all' || showcase.theme.toLowerCase().includes(filterTheme.toLowerCase());
    const matchesStyle = filterStyle === 'all' || showcase.style.toLowerCase().includes(filterStyle.toLowerCase());
      ...
      <div className="relative">
        ...
        >
          ...
          <div className="absolute top-3 right-3 flex gap-2">
            ...
            >
              <Heart 
                className={`w-4 h-4 ${favorites.includes(showcase.id) ? 'fill-red-500 text-red-500' : ''}`} 
              />
            </Button>
          </div>
        ...
        <div className="p-4">
          <div className="flex items-start justify-between mb-2">
            <div>
              <h3 className="font-semibold text-lg line-clamp-1">{showcase.venueName}</h3>
          ...
          <div className="space-y-2 mb-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-green-600">{showcase.priceRange}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
            ...
            <span className="flex items-center gap-1">
              ...
              Available {new Date(showcase.availability.nextAvailable).toLocaleDateString()}
            </span>
          </div>
  ...
  const renderVirtualTour = () => {
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-lg">{selectedShowcase.venueName}</h3>
            <p className="text-gray-600">Virtual Tour - {selectedShowcase.theme}</p>
          </div>
          <div className="flex gap-2">
            <Button size="sm" variant="outline">
...
>
  <div className="absolute inset-0 flex items-center justify-center bg-black/25">
    <div className="text-center space-y-4">
      ...
      <div>
        <h4 className="font-semibold text-xl text-white">360° Virtual Tour</h4>
        <p className="text-gray-200 max-w-md">
          Interactive virtual reality experience showing {selectedShowcase.vendorName}'s 
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <Card key={element.id} className="p-2">
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="text-right">
          <div className="font-semibold text-green-600">${element.price}</div>
  ...
  return (
    <div className="min-h-screen bg-[#111827]">
      {/* Header */}
      <div className="bg-[#1F2937] shadow-sm ">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">Natural Venue Showcase</h1>
              <p className="text-white">Immersive decor experiences in real venues</p>
            </div>
            ...
            {isVirtualTourActive ? (
              ...
              <>
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-gray-600">
                      Real decor setups in actual venues - virtually explore before you book
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredShowcases.map(showcase => renderShowcaseCard(showcase))}
                </div>
      ...
      {/* Showcase Day Banner */}
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white py-8 mt-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h2 className="text-2xl font-bold mb-2">Join Our Next Showcase Day</h2>
          <p className="text-purple-100 mb-4">
            Vendors can book time slots to set up real decor themes in partner venues
          </p>
