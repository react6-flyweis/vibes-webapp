...
import {
  Send, Users, Calendar, MapPin, Clock, Star, Heart, Share2, Download, Eye, 
  QrCode, Smartphone, Mail, MessageSquare, Instagram, Twitter, Facebook,
  Palette, Camera, Music, Gift, Crown, Zap, Sparkles, Globe, Lock,
  CheckCircle, AlertCircle, Play, Pause, Volume2, Image as ImageIcon,
  Video, ArrowRight, ArrowLeft, Plus, Edit3, Copy, ExternalLink,
  Settings, Sliders, Wand2, Bot, Gamepad2, ShoppingBag, Headphones
} from "lucide-react";
...
interface InvitationEvent {
  ...
  hostAvatar: string;
  coverImage: string;
  category: "birthday" | "wedding" | "corporate" | "party" | "cultural" | "nightclub";
  capacity: number;
  ticketPrice?: number;
  isPrivate: boolean;
...
interface InvitationTemplate {
  id: string;
  name: string;
  category: "classic" | "modern" | "cultural" | "ar-enhanced" | "nft-exclusive" | "interactive";
  thumbnail: string;
...
export type StepType = "event" | "template" | "customize" | "guests" | "preview" | "send";

export default function CompleteInviteWorkflow() {
  const { toast } = useToast();
  
  // Core workflow state
  const [currentStep, setCurrentStep] = useState<"event" | "template" | "customize" | "guests" | "preview" | "send">("event");
  const [selectedEvent, setSelectedEvent] = useState<InvitationEvent | null>(null);
  const [selectedTemplate, setSelectedTemplate] = useState<InvitationTemplate | null>(null);
  const [guestList, setGuestList] = useState<Guest[]>([]);
  const [customizations, setCustomizations] = useState<InvitationPreview["customizations"]>({
  ...
  const [guestImportMethod, setGuestImportMethod] = useState<"manual" | "csv" | "contacts" | "social">("manual");
  const [singleGuest, setSingleGuest] = useState({ name: "", email: "", phone: "", tier: "general" as const });

  // Preview and sending state
  const [previewMode, setPreviewMode] = useState<"desktop" | "mobile" | "ar" | "nft">("desktop");
  const [sendingMethod, setSendingMethod] = useState<"email" | "sms" | "social" | "qr" | "nfc">("email");
  ...
  const generateInvitationMutation = useMutation({
    mutationFn: async (data: {
      ...
      const response = await apiRequest("POST", "/api/invitations/generate", data);
      return response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const sampleEvents: InvitationEvent[] = [
    {
      ...
      description: "Join us for an unforgettable night celebrating Sarah's milestone birthday with music, dancing, and great vibes!",
      date: "2025-07-15",
      time: "20:00",
    ...
    },
    {
      ...
      description: "Celebrating breakthrough achievements and innovation in technology with industry leaders and visionaries.",
      date: "2025-08-10",
      time: "18:30",
      ...
      dresscode: "Black Tie",
      amenities: ["Networking Lounge", "Awards Ceremony", "Gourmet Dining", "Live Entertainment"],
      vibesTags: ["professional", "networking", "elegant", "awards", "innovation"],
...
,
      ...
      features: ["3D Neon Animations", "AR Party Preview", "Interactive Elements", "Sound Integration"],
      isPremium: true,
      arElements: ["Floating Neon Signs", "3D Party Scene", "Interactive DJ Booth", "Virtual Confetti"],
      interactiveFeatures: ["Tap to RSVP", "AR Venue Tour", "Music Preview", "Guest Messages"],
    ...
    },
    {
      ...
      features: ["Multi-language Support", "Cultural Elements", "Traditional Patterns", "Local Customs"],
      isPremium: false,
      culturalRegion: "Global",
      interactiveFeatures: ["Cultural Quiz", "Traditional Music", "Language Selection"],
...
,
      ...
      features: ["Blockchain Authentication", "NFT Pass Generation", "Exclusive Access", "Digital Collectibles"],
      isPremium: true,
      nftIntegration: true,
      interactiveFeatures: ["NFT Minting", "Wallet Connect", "Token Gating", "Exclusive Perks"],
      gamificationLevel: "advanced",
    },
  ];
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <div className="max-w-2xl mx-auto mb-8">
            ...
            <div className="flex justify-between text-sm text-gray-500">
              <span className={currentStep === "event" ? "text-purple-600 font-medium" : ""}>Event Details</span>
              <span className={currentStep === "template" ? "text-purple-600 font-medium" : ""}>Choose Template</span>
              <span className={currentStep === "customize" ? "text-purple-600 font-medium" : ""}>Customize</span>
              <span className={currentStep === "guests" ? "text-purple-600 font-medium" : ""}>Guest List</span>
              <span className={currentStep === "preview" ? "text-purple-600 font-medium" : ""}>Preview</span>
              <span className={currentStep === "send" ? "text-purple-600 font-medium" : ""}>Send</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center mb-8">
          <div className="flex space-x-2">
            {["event", "template", "customize", "guests", "preview", "send"].map((step, index) => (
              <Button
                key={step}
                ...
                <Tabs defaultValue="new" className="w-full">
                  ...
                  <TabsContent value="new" className="space-y-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <Select 
                            ...
                            onValueChange={(value: any) => setEventForm(prev => ({ ...prev, category: value }))}
                          >
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                    ...
                    <div className="flex justify-end">
                      ...
                      >
                        {createEventMutation.isPending ? "Creating..." : "Create Event & Continue"}
          ...
          {currentStep === "template" && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select from our collection of professional invitation templates
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    >
                      <div className="aspect-video relative overflow-hidden rounded-t-lg">
                        ...
                        <div className="absolute top-2 left-2">
                          <Badge variant="secondary" className={template.isPremium ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800"}>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 text-sm text-gray-600">
                            ...
                            {template.gamificationLevel && <Gamepad2 className="w-4 h-4" />}
                          </div>
          ...
          {currentStep === "customize" && selectedTemplate && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <CardContent className="space-y-6">
                      ...
                      {[
                        ...
                        <div
                          ...
                          className={`aspect-square rounded-lg cursor-pointer ring-2 ${
                            JSON.stringify(customizations.colors) === JSON.stringify(colors)
                              ? "ring-gray-900"
                              : "ring-transparent"
                          }`}
                          style={{ background: `linear-gradient(45deg, ${colors[0]}, ${colors[1]})` }}
                          onClick={() => setCustomizations(prev => ({ ...prev, colors }))}
                        />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedTemplate.arElements && (
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            setCustomizations(prev => ({ ...prev, arExperience: checked }))
                          }
                        />
                      </div>
                    )}
                    ...
                    {selectedTemplate.nftIntegration && (
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            setCustomizations(prev => ({ ...prev, nftPassPreview: checked }))
                          }
                        />
                      </div>
                    )}
                    ...
                    {selectedTemplate.gamificationLevel && (
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            setCustomizations(prev => ({ ...prev, gamificationElements: checked }))
                          }
                        />
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) => 
                          setCustomizations(prev => ({ ...prev, sustainabilityBadges: checked }))
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                      Let AI optimize your invitation design based on your event type and audience
                    </p>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="aspect-[3/4] bg-gradient-to-br from-purple-100 to-blue-100 dark:from-purple-900 to-blue-900 rounded-lg p-6 relative overflow-hidden">
                    ...
                    >
                      ...
                      <div className="relative z-10 p-6 text-white h-full flex flex-col justify-between">
                        <div>
                          <h3 className="text-2xl font-bold mb-2">{selectedEvent?.title || "Your Event"}</h3>
                          <p className="text-white/90 text-sm mb-4">{selectedEvent?.description || "Event description here..."}</p>
                        </div>
                        
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            <Calendar className="w-4 h-4" />
                            {selectedEvent?.date || "Date"} at {selectedEvent?.time || "Time"}
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <MapPin className="w-4 h-4" />
          ...
          {currentStep !== "event" && currentStep !== "template" && (
            <div className="flex justify-between mt-8">
              <Button 
                variant="outline" 
                onClick={() => {
                  const steps = ["event", "template", "customize", "guests", "preview", "send"];
