...
export default function CateringMarketplace() {
  ...
  const { data: cateringMenus = [], isLoading } = useQuery({
    queryKey: ["/api/catering/menus", selectedCategory, selectedCuisine, searchQuery],
  });
  ...
  const filteredMenus = cateringMenus.filter((menu: CateringMenu) => {
    ...
    const matchesCuisine = selectedCuisine === "all" || menu.cuisine === selectedCuisine;
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "main-course": return <ChefHat className="h-4 w-4" />;
      case "desserts": return <ShoppingCart className="h-4 w-4" />;
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-900 via-red-900 to-pink-900 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-4">Catering Marketplace</h1>
          <p className="text-xl text-orange-100 max-w-2xl mx-auto">
            Discover amazing catering menus from top chefs. From intimate dinners to grand celebrations.
          </p>
        </div>
        ...
        {/* Featured Menus */}
        {filteredMenus.filter((menu: CateringMenu) => menu.featured).length > 0 && (
          <div className="mb-8">
                ...
                .map((menu: CateringMenu) => (
                  <Card key={menu.id} className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 backdrop-blur border-yellow-300/30">
                    <CardHeader className="pb-3">
                      ...
                      <CardTitle className="text-white text-lg">{menu.name}</CardTitle>
                      <p className="text-orange-200">by Chef {menu.chef}</p>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex flex-wrap gap-1">
                          <Badge variant="outline" className="text-xs border-orange-300 text-orange-200">
                            {menu.cuisine}
                          </Badge>
                          {menu.dietary.slice(0, 2).map((diet, index) => (
                            <Badge key={index} variant="outline" className="text-xs border-green-300 text-green-200">
                              {diet}
                            </Badge>
                          ))}
                        </div>
                        
                        <p className="text-orange-100 text-sm line-clamp-2">{menu.description}</p>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-orange-200 text-sm">{menu.cuisine}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="flex items-center gap-1">
                        <Star className="h-4 w-4 text-yellow-400 fill-current" />
                        <span className="text-white font-semibold">{menu.rating}</span>
                      </div>
                      <div className="text-orange-200 text-sm">{menu.reviews} reviews</div>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
            ...
            <Card className="bg-white/10 backdrop-blur border-white/20 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <CardHeader>
                ...
                <p className="text-orange-200">
                  by Chef {selectedMenu.chef} • ${selectedMenu.pricePerPerson}/person
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {selectedMenu.items.map((item, index) => (
                      <div key={index} className="text-orange-200 text-sm">• {item}</div>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-orange-100 text-sm font-medium">Event Date</label>
                    <Input
                      type="date"
                  ...
                  <div>
                    <label className="text-orange-100 text-sm font-medium">Number of Guests</label>
                    <Input
                      type="number"
                  ...
                  <div>
                    <label className="text-orange-100 text-sm font-medium">Serving Style</label>
                    <Select 
                      value={orderDetails.servingType} 
                      onValueChange={(value) => setOrderDetails(prev => ({ ...prev, servingType: value }))}
                    >
                  ...
                  <div>
                    <label className="text-orange-100 text-sm font-medium">Delivery Address</label>
                    <Input
                ...
                <div>
                  <label className="text-orange-100 text-sm font-medium">Dietary Restrictions</label>
                  <Input
                    placeholder="Any allergies or dietary requirements..."
                    value={orderDetails.dietaryRequests}
                    onChange={(e) => setOrderDetails(prev => ({ ...prev, dietaryRequests: e.target.value }))}
                    className="bg-white/10 border-white/20 text-white placeholder:text-orange-200"
                  />
                </div>
                
                <div>
                  <label className="text-orange-100 text-sm font-medium">Special Instructions</label>
                  <textarea
                ...
                {orderDetails.guestCount && parseInt(orderDetails.guestCount) >= selectedMenu.minOrder && (
                  <div className="p-4 bg-orange-500/20 rounded-lg border border-orange-300/30">
                    <div className="space-y-2">
                      <div className="flex justify-between text-white">
                        <span>Subtotal ({orderDetails.guestCount} guests):</span>
                        <span>${(parseInt(orderDetails.guestCount) * selectedMenu.pricePerPerson).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-orange-200">
                        <span>Platform fee (15%):</span>
                        <span>${(parseInt(orderDetails.guestCount) * selectedMenu.pricePerPerson * 0.15).toFixed(2)}</span>
                      </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button
                    onClick={handleOrder}
                    disabled={orderMutation.isPending || !orderDetails.guestCount || parseInt(orderDetails.guestCount) < selectedMenu.minOrder}
