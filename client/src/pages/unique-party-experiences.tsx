...
export default function UniquePartyExperiences() {
  ...
  const uniquePartyEvents = allEvents.filter((event: any) => 
    ['party-bus', 'cruise', 'yacht', 'rooftop', 'warehouse', 'beach', 'pool', 'boat', 'private-jet', 'train', 'underground'].includes(event.category)
  );
  ...
  const cities = Array.from(new Set(uniquePartyEvents.map((event: any) => event.city).filter(Boolean)));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-purple-200 max-w-3xl mx-auto">
            Discover extraordinary party venues and experiences - from luxury yacht cruises to underground warehouse raves
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((event: any) => {
            ...
            return (
              <Card key={event.id} className="bg-white/10 backdrop-blur border-white/20 hover:bg-white/15 transition-all overflow-hidden group">
                ...
                <CardContent>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      Book Now - From ${typeof event.price === 'object' ? event.price.min : event.price}
                    </Button>
                  </div>
                </CardContent>
              </Card>
        ...
        {filteredEvents.length === 0 && (
          <div className="text-center text-white py-12">
            ...
            <p className="text-purple-200">Try adjusting your search criteria or check back later for new events</p>
          </div>
        )}

        {/* Call to Action */}
        <div className="mt-16 text-center">
          <Card className="bg-gradient-to-r from-purple-600/20 to-blue-600/20 border-purple-300/30 backdrop-blur">
            <CardContent className="p-8">
              <h3 className="text-2xl font-bold text-white mb-4">
                Want to host your own unique party experience?
              </h3>
              <p className="text-purple-200 mb-6">
                Create unforgettable memories with our specialized event planning services
              </p>
              <Button 
                size="lg" 
